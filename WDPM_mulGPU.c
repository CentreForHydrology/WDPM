...
int main(int argc, char *argv[])
{
	...
	cols_base = (numcols+2) / num_devices;
	if (cols_base%3 == 0)
	{
	...
	if (num_devices > 1)
	{
		for(i=0;i<num_devices; i++)
		{
			if(i == 0)
			{
				section_sizes[i] = cols_base;
				iteration_cols[i] = cols_base - 2;
				offsets[i] = 0;
				printf("%d \n", offsets[0]);
			}
			if(i == (num_devices-1))
			{
				offsets[0] = 0;
				section_sizes[i] = numcols+2-(num_devices-1)*cols_base;
