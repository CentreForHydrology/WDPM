...
void runoffadd(__global double *bigwater, __global double *bigdem, int centerrow, 
  ...
  {   
    ...
    { 
      ...
      if (((rowloc != centerrow) || (colloc != centercol)) && (bigdem[rowloc+(numrows+2)*colloc] > missingvalue))
      {
...
__kernel void add(__global double *bigwater, __global double *bigdem, const double missingvalue, 
  ...
  row = (oi-off)+off*row1;
  col = (oj-off)+off*col1;
  if (row>=1 && row<=numrows && col>=1 && col<=numcols && bigwater[row+(numrows+2)*col]>0.0 && bigdem[row+(numrows+2)*col]>missingvalue )
  {
