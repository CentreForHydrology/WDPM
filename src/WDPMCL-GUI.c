...
cl_device_id create_device(int opencl) {
   ...
   if (opencl == 1){
      ...
      if(err < 0) {
	  perror("Couldn't access any devices (Try switch to GPU)");
	  exit(1);   
      }
   }
...
{ 
	...
	char subtractstring[10], rofstring[10], CpuString[10], GpuString[10], FlagString[10];
    ...
    int slice, boundary, cpu, gpu, myflag;
	...
	switch (argc)
	{
		...
		{               
			...
			if (i==11){ strcpy(FlagString, input); }
		    i++;                 
		}            
		fclose(filez);
		break;
	...
	switch (argc)
	{
		...
		{               
			...
			if (i==10){ strcpy(FlagString, input); }
		    i++;                 
		}            
		fclose(filez);
		break;
